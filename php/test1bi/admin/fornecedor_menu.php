<?php
    ...
    } else
    {
        ...
        $q = "SELECT COUNT(id) From fornecedor WHERE nome like '%$where%'";
    ...
    $q = "SELECT id, nome, cidade, estado, data_inclusao 
            FROM fornecedor WHERE nome like '%$where%' 
    ...
    if(mysqli_num_rows($r) > 0) {
            ...
            <table class='table table-stripped'>
                <thead>
                    <tr>
                        ...
                        <th><a href='fornecedor_menu.php?ordem=nome'>Nome</a></th>
                        <th><a href='fornecedor_menu.php?ordem=cidade'>Cidade</a></th>
                        <th><a href='fornecedor_menu.php?ordem=estado'>Estado</a></th>
                        <th><a href='fornecedor_menu.php?ordem=d'>Data de Inclusão</a></th>
                        <th>Ações</th>
                    </tr>
                </thead>
                <tbody>";
        while ($row = mysqli_fetch_array($r, MYSQLI_ASSOC))
        {
            $saida .= "<tr>
                            ...
                            "<td>
                                <a href='fornecedor_alt.php?id=".$row['id']."' class='btn btn-sm btn-warning'> Editar </a>
                                <a href='fornecedor_exc.php?id=".$row['id']."' class='btn btn-sm btn-danger'> Excluir </a>
                            </td>
                        </tr>";
    ...
    if($pagina > 1) {
        ...
        if ($pagina_correta != 1) {
            ...
            <a class="page-link" href="fornecedor_menu.php?s=' . ($inicio - $exiba) . '&p=' . $pagina .
    ...
    for ($i = 1; $i <= $pagina; $i++) {
        if ($i != $pagina_correta) {
            $pag .= '<li class="page-item">
            <a class="page-link" href="fornecedor_menu.php?s=' . ($exiba * ($i- 1)) . '&p=' . $pagina .
...
?>
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
        <h1 class="h2">Menu do Fornecedor</h1>
    </div>

    <div class="row mb-3">
        <div class="col-md-9">
            <div class="input-group">
                <input type="text" class="form-control" placeholder="Pesquise o nome do fornecedor" id="busca" />
                <div class="input-group-append">
                    <a href="#" class="btn btn-primary" onclick="this.href='fornecedor_menu.php?q='+ document.getElementById('busca').value">
